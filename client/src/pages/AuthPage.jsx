...
export default function AuthPage({ onAuth }) {
 ...
 const submit = async () => {
    ...
    try {
      // AUTO-DETECT: Use localhost if you are in VS Code, otherwise use Render
      ...
      setError(e.message === "Failed to fetch" ? "Server waking up. Please retry in 20s." : e.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="full-page">
      ...
      <div className="anim-up" style={{ textAlign: 'center', marginBottom: 44 }}>
        ...
        <h1 className="font-d" style={{ fontSize: 'clamp(44px,9vw,72px)', fontWeight: 800, lineHeight: 0.92, letterSpacing: '-0.025em', color: '#fff' }}>SENTINEL</h1>
        <h1 className="font-d" style={{ fontSize: 'clamp(44px,9vw,72px)', fontWeight: 800, lineHeight: 0.92, letterSpacing: '-0.025em', color: 'var(--green)', marginBottom: 20 }}>ASSEMBLY</h1>
        <p style={{ color: 'var(--text-2)', fontSize: 14, fontWeight: 300, lineHeight: 1.6 }}>Self-governing AI content governance engine.<br />Three agents. One truth.</p>
      </div>

      <div className="card anim-up-1" style={{ width: '100%', maxWidth: 400, padding: 28 }}>
        <div className="tabs" style={{ marginBottom: 24 }}>
          <button className={`tab-item ${mode === 'login' ? 'active' : ''}`} onClick={() => { setMode('login'); setError(''); setPass(''); }}>Sign In</button>
          <button className={`tab-item ${mode === 'register' ? 'active' : ''}`} onClick={() => { setMode('register'); setError(''); }}>Create Account</button>
        </div>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          <input className="input" type="password" placeholder="Password" value={password} onChange={e => setPass(e.target.value)} onKeyDown={e => e.key === 'Enter' && submit()} />
        </div>

        {showGenerated && mode === 'register' && password && (
          <div style={{ marginBottom: 16, padding: 12, background: 'rgba(0, 255, 136, 0.08)', border: '1px solid rgba(0,255,136,0.4)', borderRadius: 8 }}>
            <div style={{ color: 'var(--green)', fontSize: 13, marginBottom: 6 }}>✅ Strong password auto-generated:</div>
            <div style={{ fontFamily: 'monospace', fontSize: 15, color: '#fff', wordBreak: 'break-all' }}>{password}</div>
          </div>
        )}

        {error && <div className="form-error" style={{ marginBottom: 14 }}><span>⚠</span> {error}</div>}
        ...
        <button className="btn-ghost" style={{ width: '100%', justifyContent: 'center' }} onClick={loadDemo}>Load demo credentials</button>
      </div>

      <div className="anim-up-2" style={{ display: 'flex', gap: 10, marginTop: 40, flexWrap: 'wrap', justifyContent: 'center' }}>
        {["Three Minds, One Truth", "Vault-Locked Security", "Living Archive", "Recursive Auditing"].map((text, i) => (
          <span key={i} className="chip chip-dim" style={{ fontSize: 11.5, padding: '8px 16px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)' }}>{text}</span>
        ))}
      </div>
    </div>
  );
...
//   try {
//     const BASE_URL = "https://sentinel-factory-1.onrender.com";
...
//   } catch (e) {
//     setError(e.message || "Server not reachable");
//   } finally {
//     setLoading(false);
//   }
// };
